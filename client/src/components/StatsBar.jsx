export default function StatsBar({ stats }) {
  const items = [
    ...
    { label: 'Queue', value: stats.queue_size ?? 0, color: stats.queue_size > 5 ? 'text-red' : '' },
  ];

  return (
    <div className="flex gap-2 flex-wrap">
      {items.map(item => (
        <div key={item.label} className="bg-bg2 border border-bdr rounded-lg px-4 py-2.5 text-center min-w-[80px] flex-1">
